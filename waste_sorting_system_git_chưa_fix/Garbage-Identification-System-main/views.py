import os #path 
import random #random number ngẫu nhiên 

from app import app, db  #app và connect data from app
from model import * #import all model from file model.py 
from flask import request, url_for, redirect, flash, render_template, jsonify #import functions from flaaks
from flask_security import SQLAlchemyUserDatastore, Security, current_user, login_required #flask_security để quản lý người dùng và xác thực.
from flask_security.utils import login_user, logout_user #login_user và logout_user
import forms
import datetime, time #xử lý time
from shibie import get_detection
#: Import hàm get_detection từ file shibie.py để thực hiện nhận dạng (có thể là nhận dạng hình ảnh hoặc văn bản).

#connect data và xác thực user
# Setup Flask-Security
#SQLAlchemyUserDatastore để tương tác với user:tạo xóa ,xác thực
#role là quyền của user
user_datastore = SQLAlchemyUserDatastore(db, User, Role)

security = Security(app, user_datastore)
#Flask-Security manager all app and database user
...
#trả về giao diện index trong template
...
"""
 ...
 Đăng nhập
 Đăng xuất
 Quản lý quyền--'Người dùng','Quản trị viên','Root'
 Quản lý quyền người dùng 
 -- user_role (Quản trị viên/Root) 
 -- Sửa đổi/thay đổi_role 
 (thăng cấp lên quản trị viên, giáng cấp xuống người dùng thông thường) (Root)
"""



#lấy thời gian real time và trả về thành string:
def gettime():
    current_time = datetime.datetime.now()
    now_time = current_time.strftime("%Y-%m-%d %H:%M:%S")
    return now_time
""" 
Check user login thì chuyển hướng về trang chủ.
Nếu chưa, user will đăng nhập, check tên đăng nhập và mật khẩu.
Nếu trống, hàm sẽ yêu cầu nhập lại.
"""
